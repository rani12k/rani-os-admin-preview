const build = 'CR-017-DEV-DATA-MOBILE-001';
...
function counters() {
  ...
  const mapped = model();
  const items = [['control', `${mapped.length}/${universe.length}`, 'Mapped / Total'], ['unmapped', universe.length - mapped.length, 'Unmapped'], ['owner', mapped.reduce((sum, item) => sum + item.gates.length, 0), 'Owner Gates'], ['work', workRows().filter(row => inScope(appOfWork(row))).length, 'Work Records'], ['artifacts', artifacts.filter(row => inScope(appOfArtifact(row))).length, 'Artifacts']];
  return `<div class="counters">${items.map(([nav, num, label]) => `<button class="counter" data-nav="${nav}"><div class="num">${esc(num)}</div><div class="label">${esc(label)}</div></button>`).join('')}</div>`;
}
function layout(content) {
  document.body.innerHTML = `${envFrame()}${envBar()}<header><h1>RANI OS Admin</h1><div class="sub">Build ${esc(build)} · DEV data coverage + mobile recovery build · Visual identity: ${esc(visualIdentity)}</div>${appSwitch()}<div class="tabs">${[['control','Control Room'],['mapping','Mapping Coverage'],['priority','Priority Board'],['capmap','Capability Map'],['unmapped','Unmapped'],['owner','Owner Gates'],['work','Work Records'],['artifacts','Artifacts'],['versions','Version Center'],['config','Config Center'],['roles','Role / Access'],['cicd','CI/CD'],['reports','Reports']].map(([key,label]) => `<button data-tab="${key}" class="${tab === key ? 'active' : ''}">${label}</button>`).join('')}</div>${counters()}<div class="legend">${chip('done','PASS / Done')} ${chip('open','Warning / Review')} ${chip('stuck','Blocked / Stop')} ${chip('other','Info / N/A')}</div></header><main>${envCenter()}${content}</main>`;
  wire();
}
...
function control() {
  const universe = capabilityUniverse().filter(cap => inScope(cap.application));
  const mapped = model();
  return `<section><h2>Control Room</h2><div class="note">DEV recovery build. Data coverage and mobile readability are under review in DEV only.</div><div class="kpis"><div class="kpi"><div class="num">${mapped.length}/${universe.length}</div><div>Mapped / Total</div></div><div class="kpi"><div class="num">${universe.length - mapped.length}</div><div>Unmapped</div></div><div class="kpi"><div class="num">${workRows().length}</div><div>Work Records</div></div><div class="kpi"><div class="num">${artifacts.length}</div><div>Artifacts</div></div></div>${table([{ K: 'Source of Truth', V: state.system_status?.source_of_truth }, { K: 'Projection Status', V: state.system_status?.projection_status }, { K: 'Active Work', V: state.current_work_item?.active_work_item }, { K: 'Build', V: build }, { K: 'Scope', V: appScope }], [['Context','K'], ['Value','V']])}</section>`;
}
